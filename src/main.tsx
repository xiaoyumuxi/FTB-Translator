...
function App() {
  ...
  const progressTarget=useRef({done:0,total:0}); const progressDisplayed=useRef({done:0,total:0}); const progressTimer=useRef<number|null>(null);
  ...
  useEffect(()=>{
    ...
    return()=>{window.removeEventListener("error",onError);window.removeEventListener("unhandledrejection",onRejection);stopProgressAnimation()};
  },[]);
  ...
  useEffect(()=>{ const unlisten=listen<TranslationEvent>("translation-event",({payload:e})=>{
    ...
    if(e.type==="progress") { queueProgress(e.done||0,e.total||0); void frontendLog("trace","translation_progress","前端收到翻译进度",{task_id:e.task_id||"",done:e.done||0,total:e.total||0,stage:e.stage||""}); }
    if(e.type==="review_ready"&&e.cmp_path) { const draft={cmp_path:e.cmp_path,task_id:e.task_id,total_entries:e.total_entries||0,warning_count:e.warning_count||0,failed_count:e.failed_count||0};retryingRateLimited.current=false;stopProgressAnimation();setBusy(false);setProgress(100);setStage("review");setCmpDraft(draft);void loadCmpEntries(draft);setReviewPrompt(true);setLogs(v=>[...v,note("API 翻译完成，已打开可编辑校对表格，尚未覆盖任务书。")]);void frontendLog("info","cmp_review_ready","CMP 校对文件已生成",draft);notify("翻译完成，请确认是否直接覆盖"); }
    if(e.type==="done"&&e.report) { retryingRateLimited.current=false;stopProgressAnimation();setBusy(false); setProgress(100); setStage("done"); setReport(e.report); setLogs(v=>[...v,note("翻译完成，输出与备份均已写入。")]); void frontendLog("info","translation_completed","前端收到翻译完成事件",{run_id:e.run_id,total:e.report.total_entries,translated:e.report.translated_entries,failed:e.report.failed_entries.length,warnings:Object.keys(e.report.warnings).length}); notify("任务书汉化完成"); loadHistory(); }
    if(e.type==="error") { const retrying=retryingRateLimited.current;retryingRateLimited.current=false;stopProgressAnimation();setBusy(false); setStage(retrying?"review":"scanned"); void frontendLog("error","translation_failed","前端收到翻译失败事件",{task_id:e.task_id||"",error:e.message||"翻译失败"}); notify(e.message||"翻译失败"); }
  ...
  function stopProgressAnimation(){if(progressTimer.current!==null){window.clearInterval(progressTimer.current);progressTimer.current=null}}
  function resetProgress(total=0){stopProgressAnimation();progressTarget.current={done:0,total};progressDisplayed.current={done:0,total};setProgress(0);setProgressDetail({done:0,total})}
  function queueProgress(done:number,total:number){if(total<=0){resetProgress();return}const safeDone=Math.min(done,total);if(progressDisplayed.current.total!==total){stopProgressAnimation();progressDisplayed.current={done:0,total};setProgressDetail({done:0,total});setProgress(0)}progressTarget.current={done:safeDone,total};if(progressTimer.current!==null)return;progressTimer.current=window.setInterval(()=>{const target=progressTarget.current;const shown=progressDisplayed.current;if(shown.done>=target.done){stopProgressAnimation();return}const next={done:shown.done+1,total:target.total};progressDisplayed.current=next;setProgressDetail(next);setProgress(Math.min(100,Math.round(next.done/next.total*100)));if(next.done>=target.done)stopProgressAnimation()},24)}
  ...
  async function doScan(path=selectedPath){ if(!path.trim()){void frontendLog("warn","scan_rejected","扫描未开始：目录为空");return notify("请先选择整合包目录")} resetProgress();setBusy(true); setReport(null); setCmpDraft(null); setCmpEntries([]); setReviewPrompt(false); void frontendLog("info","scan_started","用户开始扫描任务书",{path}); try { const result=await call<ScanResult>("scan",{path,batch_size:settings.batch_size}); setScan(result); setSelectedPath(result.quests_dir); setStage("scanned"); setLogs([note(`已找到 ${result.entry_count} 条可翻译文本。`),note(`源目录：${result.source}`)]); void frontendLog("info","scan_completed","前端已展示扫描结果",{mode:result.mode,entries:result.entry_count,files:result.file_count}); } catch(e){setStage("error");notify(String(e))} finally{setBusy(false)} }
  async function beginTranslation(){setConfirm(false);if(!scan){void frontendLog("warn","translation_rejected","翻译未开始：没有扫描结果");return}retryingRateLimited.current=false;resetProgress();setBusy(true);setStage("running");setLogs([note("正在启动安全翻译任务…")]);void frontendLog("info","translation_started","用户确认开始翻译",{quests_dir:scan.quests_dir,provider:settings.provider});try{await invoke("start_translation",{payload:{quests_dir:scan.quests_dir,...settings}})}catch(e){void frontendLog("error","translation_start_failed","启动翻译命令失败",{error:errorText(e)});setBusy(false);setStage("scanned");notify(String(e))}}
  async function retryRateLimited(){if(!scan||!cmpDraft)return;const count=cmpEntries.filter(entry=>entry.status==="rate_limited").length;if(!count)return notify("当前没有可重试的限流条目");try{await call("cmp-save-edits",{cmp_path:cmpDraft.cmp_path,entries:cmpEntries});retryingRateLimited.current=true;resetProgress(count);setBusy(true);setStage("running");setLogs(v=>[...v,note(`正在重新调用翻译接口处理 ${count} 条限流内容…`)]);void frontendLog("info","rate_limited_retry_started","用户重试限流条目",{task_id:cmpDraft.task_id||"",cmp_path:cmpDraft.cmp_path,count});await invoke("start_translation",{payload:{quests_dir:scan.quests_dir,retry_cmp_path:cmpDraft.cmp_path,...settings}})}catch(e){retryingRateLimited.current=false;setBusy(false);setStage("review");void frontendLog("error","rate_limited_retry_failed","限流条目重试启动失败",{task_id:cmpDraft.task_id||"",error:errorText(e)});notify(String(e))}}
