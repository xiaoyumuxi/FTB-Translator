...
type CmpDraft = { cmp_path:string; total_entries:number; warning_count:number; failed_count:number };
type TranslationEvent = { type:"progress"|"log"|"done"|"review_ready"|"error"; stage?:string; done?:number; total?:number; message?:string; report?:Report; run_id?:number; cmp_path?:string; total_entries?:number; warning_count?:number; failed_count?:number };
...
function App() {
  ...
  useEffect(()=>{ const unlisten=listen<TranslationEvent>("translation-event",({payload:e})=>{
    ...
    if(e.type==="error") { setBusy(false); setStage("error"); void frontendLog("error","translation_failed","前端收到翻译失败事件",{error:e.message||"翻译失败"}); notify(e.message||"翻译失败"); }
  ...
  async function applyCmp(){if(!scan||!cmpDraft)return;setReviewPrompt(false);setBusy(true);setStage("running");setLogs(v=>[...v,"正在校验 CMP 并创建备份…"]);try{const result=await call<{report:Report;run_id:number}>("cmp-apply",{cmp_path:cmpDraft.cmp_path,quests_dir:scan.quests_dir});setBusy(false);setProgress(100);setStage("done");setReport(result.report);setLogs(v=>[...v,"CMP 校验通过，翻译结果已写入任务书。"]);void frontendLog("info","cmp_applied","CMP 已应用",{run_id:result.run_id,cmp_path:cmpDraft.cmp_path});notify("任务书汉化完成");loadHistory()}catch(e){setBusy(false);setStage("review");notify(String(e))}}
  async function openCmp(){if(!cmpDraft)return;try{await call("cmp-open",{cmp_path:cmpDraft.cmp_path})}catch(e){notify(String(e))}}
  async function exportCmp(){if(!cmpDraft)return;const target=await save({title:"导出 CMP 校对文件",defaultPath:"ftb-translation-review.cmp",filters:[{name:"FTB CMP 校对文件",extensions:["cmp"]}]});if(target)try{await call("cmp-export",{cmp_path:cmpDraft.cmp_path,path:target});notify("CMP 校对文件已导出")}catch(e){notify(String(e))}}
  async function chooseCmp(){const value=await open({multiple:false,directory:false,title:"选择 CMP 校对文件",filters:[{name:"FTB CMP 校对文件",extensions:["cmp"]}]});if(typeof value==="string"){setCmpDraft(cmpDraft?{...cmpDraft,cmp_path:value}:{cmp_path:value,total_entries:scan?.entry_count||0,warning_count:0,failed_count:0});setStage("review");setReviewPrompt(false);notify("已选择 CMP 校对文件")}}
  ...
  return <div className="app-shell">
    ...
    {reviewPrompt&&cmpDraft&&<CmpDecisionDialog draft={cmpDraft} onReview={()=>setReviewPrompt(false)} onApply={applyCmp}/>}
