...
本报告审计的是 2026-07-13 当时“整条字符串交给 Google 网页翻译、通过格式守卫后直接写回”的历史结果，不是当前 CMP 校对流程的新基准。当前实现已经加入 JSON 富文本展示字段拆分、严格占位符恢复、CMP 英文 → 中文人工校对，以及确认后重新校验和写回，目标是阻止富文本和文件结构损坏。

这些结构安全改进不能自动修复术语、语境和表达质量，也不能据此提高本报告的 45%–55% 严格可发布准确率估算。只有使用当前版本重新记录输入规模、缓存状态、提供商、批大小、有效并发、耗时、失败数、CMP 人工修改情况，并完成同口径抽检后，才能发布新的准确度或格式守卫数据。下文的问题统计和案例继续作为历史基线保留。
...
4. 先解析 JSON 富文本，只翻译允许的展示字段，再重新序列化和验证。当前实现已加入该结构保护，但仍需用真实整合包重新基准测试。
5. 增加 `item → 物品`、`block → 方块`、`chunk → 区块`、`tick → 游戏刻`、`vanilla → 原版` 等语义规则。
6. 标题和任务说明使用不同的翻译提示与质量标准。
7. 对重复原文使用翻译记忆，禁止同源异译。
8. 使用 CMP 进行第二阶段术语修正与语义审校，重点检查否定、数字、方向、单位、快捷键和操作动词。当前实现已提供校对载体，实际发布仍需要人工完成审校。
